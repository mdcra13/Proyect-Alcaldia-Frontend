...
export default function ITGestionDepartamentos() {
  ...
  const { departamentos, addDepartamento, updateDepartamento, toggleDepartamentoActivo } =
  ...
  // Ref para restaurar foco al botón que abrió el modal
  const triggerRef = useRef<HTMLButtonElement | null>(null)

  const filteredDepartamentos = useMemo(() => {
    const query = searchTerm.trim().toLowerCase()
    if (!query) return departamentos
    return departamentos.filter(d => d.nombre.toLowerCase().includes(query))
  }, [departamentos, searchTerm])

  const getAssignedUsersCount = (id: string) =>
    users.filter(u => u.departamentoId === id && u.role === 'departamento').length

  const getActiveAssignedUsersCount = (id: string) =>
    users.filter(u => u.departamentoId === id && u.role === 'departamento' && u.status === 'active').length
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (editingDepartamento) {
      ...
      addDepartamento({ nombre, descripcion: descripcion || undefined })
    ...
    <AppLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button
            type="button"
            onClick={e => handleOpenCreateModal(e.currentTarget)}
            ...
            <table className="w-full" aria-label="Listado de departamentos">
              <thead className="border-b bg-muted/50">
                <tr>
                  ...
                  <th scope="col" className="p-4 text-left font-medium text-muted-foreground">Descripción</th>
              ...
              <tbody>
                {filteredDepartamentos.length === 0 ? (
                  ...
                  filteredDepartamentos.map(departamento => {
                    ...
                    return (
                      <tr key={departamento.id} className="border-b transition-colors hover:bg-muted/30">
                        <td className="p-4">
                          <div className="flex items-center gap-2">
                            <Building2 className="h-4 w-4 text-muted-foreground" aria-hidden="true" focusable="false" />
                            <span className="font-medium text-foreground">{departamento.nombre}</span>
                          </div>
                        </td>
                        <td className="p-4">
                        ...
                        <td className="p-4">
                          <div className="flex justify-end">
                            <button
                              type="button"
                              onClick={e => handleOpenEditModal(departamento, e.currentTarget)}
        ...
        {isModalOpen && (
          <AccessibleDialog
            ...
            className="modal-card max-w-130"
          >
            <div className="modal-header">
              <div>
                <h2 id="departamento-modal-title" className="font-serif text-lg font-semibold text-foreground">
              ...
              <div className="grid gap-4 p-6">
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    onChange={e => setFormData(c => ({ ...c, descripcion: e.target.value }))}
                ...
                {editingDepartamento && (
                  <div className="flex items-center justify-between rounded-lg border border-border p-4">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Alternar activo/inactivo</p>
                    </div>
                    <button
                      type="button"
                      onClick={() => {
                        if (formData.activo) {
                          const count = getActiveAssignedUsersCount(editingDepartamento.id)
                          if (count > 0) {
                            toast.error(
                              `No se puede desactivar ${editingDepartamento.nombre} porque tiene ${count} usuario(s) activo(s) asignado(s).`,
                            )
                            return
                          }
                        }
                        setFormData(c => ({ ...c, activo: !c.activo }))
                      }}
